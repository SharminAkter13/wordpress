<div class="demo-content">
    ...
    $ecommerce_shop_demo_import_completed = get_option('ecommerce_shop_demo_import_completed', false);

    // If the demo import is completed, display the "View Site" button
    if ($ecommerce_shop_demo_import_completed) {
      echo '<p class="notice-text">' . esc_html__('Your demo import has been completed successfully.', 'e-commerce-shop') . '</p>';
      echo '<a href="' . esc_url(home_url()) . '" class="view-btn" target="_blank">' . esc_html__('VIEW SITE', 'e-commerce-shop') . '</a>';
    }
    ...
    if (!is_plugin_active('woocommerce/woocommerce.php')) {
      ...
      $ecommerce_shop_plugin_slug = 'woocommerce';
      $ecommerce_shop_plugin_file = 'woocommerce/woocommerce.php';

      // Check if plugin is installed
      $ecommerce_shop_installed_plugins = get_plugins();
      if (!isset($ecommerce_shop_installed_plugins[$ecommerce_shop_plugin_file])) {
          ...
          // Install the plugin
          $ecommerce_shop_upgrader = new Plugin_Upgrader();
          $ecommerce_shop_upgrader->install('https://downloads.wordpress.org/plugin/woocommerce.latest-stable.zip');
      }
      // Activate the plugin
      activate_plugin($ecommerce_shop_plugin_file);
    }

    // --- Menu ---
    $ecommerce_shop_primary_menu_name = 'Main Menu';
    $ecommerce_shop_primary_menu_location = 'primary';
    $ecommerce_shop_primary_menu_exists = wp_get_nav_menu_object($ecommerce_shop_primary_menu_name);

    if (!$ecommerce_shop_primary_menu_exists) {
        // Create the left menu
        $ecommerce_shop_primary_menu_id = wp_create_nav_menu($ecommerce_shop_primary_menu_name);

        // Create and assign the Home page
        $ecommerce_shop_home_page_id = wp_insert_post(array(
            'post_type'     => 'page',
            'post_title'    => 'Home',
            'post_content'  => '',
            'post_status'   => 'publish',
        ...
        add_post_meta($ecommerce_shop_home_page_id, '_wp_page_template', 'page-template/custom-home-page.php');
        update_option('page_on_front', $ecommerce_shop_home_page_id);
        update_option('show_on_front', 'page');

        // Add Home page to the left menu
        wp_update_nav_menu_item($ecommerce_shop_primary_menu_id, 0, array(
            'menu-item-title'     => __('Home', 'e-commerce-shop'),
            'menu-item-classes'   => 'home',
            'menu-item-url'       => home_url('/'),
            'menu-item-status'    => 'publish',
            'menu-item-object-id' => $ecommerce_shop_home_page_id,
            'menu-item-object'    => 'page',
            'menu-item-type'      => 'post_type',
        ));

        // Create and assign the About Us page
        $ecommerce_shop_about_us_page_id = wp_insert_post(array(
            'post_type'     => 'page',
            'post_title'    => 'About Us',
        ...
        wp_update_nav_menu_item($ecommerce_shop_primary_menu_id, 0, array(
            'menu-item-title'     => __('About Us', 'e-commerce-shop'),
            'menu-item-classes'   => 'about-us',
            'menu-item-url'       => home_url('/about-us/'),
            'menu-item-status'    => 'publish',
            'menu-item-object-id' => $ecommerce_shop_about_us_page_id,
            'menu-item-object'    => 'page',
            'menu-item-type'      => 'post_type',
        ));

        // Assign left menu to its location
        $ecommerce_shop_locations = get_theme_mod('nav_menu_locations', array());
        $ecommerce_shop_locations[$ecommerce_shop_primary_menu_location] = $ecommerce_shop_primary_menu_id;
        set_theme_mod('nav_menu_locations', $ecommerce_shop_locations);
    }  

    // Set the demo import completion flag
    update_option('ecommerce_shop_demo_import_completed', true);

    // Display success message and "View Site" button
    echo '<p class="notice-text">' . esc_html__('Your demo import has been completed successfully.', 'e-commerce-shop') . '</p>';
    echo '<span><a href="' . esc_url(home_url()) . '" class="view-btn" target="_blank">' . esc_html__('VIEW SITE', 'e-commerce-shop') . '</a></span>';

    //end

    // Topbar Section
    set_theme_mod( 'bb_ecommerce_store_contact', '(512) 215 2589' );
    ...
    for($bb_ecommerce_store_i=1;$bb_ecommerce_store_i<=4;$bb_ecommerce_store_i++){
       $bb_ecommerce_store_slider_title = 'SEASON SALE';
       $bb_ecommerce_store_slider_content = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit sed do eiusmod.';
          // Create post object
    ...
    $bb_ecommerce_store_title_array = array(
        ...
        // Shop Now Section
        set_theme_mod( 'ecommerce_shop_title', 'GIFTS FOR KIDS BIRTHDAY' );
        set_theme_mod( 'ecommerce_shop_linkname', 'Shop now' );
        set_theme_mod( 'ecommerce_shop_link', '#' );
        set_theme_mod( 'ecommerce_shop_image', get_theme_file_uri().'/images/gift.jpg' );
    }
    ?>

	<p><strong><?php esc_html_e('Reminder : ', 'e-commerce-shop'); ?></strong><?php esc_html_e('Backup your site before importing. This process will overwrite existing E-commerce Shop settings.', 'e-commerce-shop'); ?></p>

    <form action="<?php echo esc_url(home_url()); ?>/wp-admin/themes.php?page=ecommerce_shop_guide" method="POST" onsubmit="return validate(this);">
    <?php if (!get_option('ecommerce_shop_demo_import_completed')) : ?>
        <form method="post">
            <input class= "run-import" type="submit" name="submit" value="<?php esc_attr_e('Run Importer','e-commerce-shop'); ?>" class="button button-primary button-large">
        </form>
    <?php endif; ?>
    </form>
