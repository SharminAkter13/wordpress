...
function ecommerce_shop_setup() {
	...
	if ( is_admin() && ('themes.php' == $pagenow) && isset( $_GET['activated']) ) {
		add_action( 'admin_notices', 'ecommerce_shop_activation_notice' );
	}
}
...
function ecommerce_shop_activation_notice() {
	echo '<div class="notice notice-info is-dismissible get-started">';
		echo '<div class="notice-box">';
			echo '<h1>'. esc_html__( 'Grateful to have you with ThemeShopy!', 'e-commerce-shop' ) .'</h1>';
			echo '<p>'. esc_html__( 'We are sincerely obliged to offer our best services to you. Please proceed towards welcome page and give us the privilege to serve you.', 'e-commerce-shop' ) .'</p>';
			echo '<p class="get-start-btn"><a href="'. esc_url( admin_url( 'themes.php?page=ecommerce_shop_guide' ) ) .'" class="button button-primary">'. esc_html__( 'Get Started with Demo', 'e-commerce-shop' ) .'</a></p>';
			echo '<p class="get-start-btn"><a href="'. esc_url('https://www.themeshopy.com/themes/ecommerce-store-wordpress-theme/') .'" class="button buy-button" target="_blank">'. esc_html__( 'Buy Now', 'e-commerce-shop' ) .'</a></p>';
		echo '</div>';
		echo '<div class="notice-img">';
			echo '<img src="' . esc_url(get_template_directory_uri()) . '/inc/admin/images/bundle-img.png">';
		echo '</div>';
	echo '</div>';
}
...
function ecommerce_shop_customize_register() {     
	...
	$wp_customize->remove_setting( 'bb_ecommerce_store_theme_color_premium_info' );
	$wp_customize->remove_control( 'bb_ecommerce_store_theme_color_premium_info' );
} 
add_action( 'customize_register', 'ecommerce_shop_customize_register', 11 );

function ecommerce_shop_links_setup() {
	define('ECOMMERCE_SHOP_BUY_NOW',__('https://www.themeshopy.com/themes/ecommerce-store-wordpress-theme/', 'e-commerce-shop'));
	define('ECOMMERCE_SHOP_LIVE_DEMO',__('https://www.themeshopy.com/ecommerce-store-wordpress-theme/', 'e-commerce-shop'));
	define('ECOMMERCE_SHOP_CONTACT',__('https://wordpress.org/support/theme/e-commerce-shop/', 'e-commerce-shop'));
	define('ECOMMERCE_SHOP_PRO_DOC',__('https://themeshopy.com/demo/docs/bb-ecommerce-store/', 'e-commerce-shop'));
...
add_action('after_setup_theme', 'ecommerce_shop_links_setup');

add_action( 'init', 'ecommerce_shop_remove_action');
function ecommerce_shop_remove_action() {
    remove_action( 'admin_menu','bb_ecommerce_store_abouttheme' );
    remove_action( 'admin_notices','bb_ecommerce_store_activation_notice' );
}
...
function ecommerce_shop_customizer ( $wp_customize ) {
	...
	$wp_customize->add_control('ecommerce_shop_sec_premium_info',array(
		...
		'description' => "<ul><li>". esc_html('Includes settings to set section title.','e-commerce-shop') ."</li><li>". esc_html('Contains settings for the background colour.','e-commerce-shop') ."</li><li>". esc_html('Contains options for background images.','e-commerce-shop') ."</li><li>". esc_html('You can change the font family and colours of heading.','e-commerce-shop') ."</li><li>". esc_html('And so on...','e-commerce-shop') ."</li></ul><a target='_blank' href='". esc_url(ECOMMERCE_SHOP_BUY_NOW) ." '>". esc_html('Upgrade to Pro','e-commerce-shop') ."</a>",
	...
	$wp_customize->add_setting( 'ecommerce_shop_slider_theme_color_next', array(
	    'default' => '',
	    'sanitize_callback' => 'sanitize_hex_color'
  	));
  	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'ecommerce_shop_slider_theme_color_next', array(
  		'label' => __('Color Option', 'e-commerce-shop' ),
  		'description' => __('One can change complete theme color on just one click.', 'e-commerce-shop'),
	    'section' => 'bb_ecommerce_store_theme_color_option',
	    'settings' => 'ecommerce_shop_slider_theme_color_next',
  	)));
}
