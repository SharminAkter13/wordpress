...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
  ...
  <div class="metabox">
    <?php if( get_theme_mod( 'bb_ecommerce_store_date_hide',true) != '') { ?>
      <span class="entry-date me-1"><i class="<?php echo esc_attr(get_theme_mod('bb_ecommerce_store_date_icon','fa fa-calendar')); ?>"></i><a href="<?php echo esc_url( get_day_link( $archive_year, $archive_month, $archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span><span class="ms-1"><?php echo esc_html( get_theme_mod('bb_ecommerce_store_metabox_separator_blog_post','|') ); ?></span>
    <?php } ?>
    <?php if( get_theme_mod( 'bb_ecommerce_store_author_hide',true) != '') { ?>
      <span class="entry-author mx-1"><i class="<?php echo esc_attr(get_theme_mod('bb_ecommerce_store_author_icon','fas fa-user')); ?>"></i><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?></a></span><span class="ms-1"><?php echo esc_html( get_theme_mod('bb_ecommerce_store_metabox_separator_blog_post','|') ); ?></span>
    <?php } ?>
    <?php if( get_theme_mod( 'bb_ecommerce_store_comment_hide',true) != '') { ?>
      <span class="entry-comments mx-1"><i class="<?php echo esc_attr(get_theme_mod('bb_ecommerce_store_comment_icon','fas fa-comments')); ?>"></i> <?php comments_number( __('0 Comment', 'bb-ecommerce-store'), __('0 Comments', 'bb-ecommerce-store'), __('% Comments', 'bb-ecommerce-store') ); ?> </span><span class="ms-1"><?php echo esc_html( get_theme_mod('bb_ecommerce_store_metabox_separator_blog_post','|') ); ?></span>
    <?php } ?>
    <?php if( get_theme_mod( 'bb_ecommerce_store_time_hide',true) != '') { ?>
      <span class="entry-time mx-1"><i class="<?php echo esc_attr(get_theme_mod('bb_ecommerce_store_time_icon','fas fa-clock')); ?>"></i> <?php echo esc_html( get_the_time() ); ?></span>
    <?php }?>
  </div>
  ...
  <?php } ?>
  <?php if( get_theme_mod( 'bb_ecommerce_store_show_post_category',true) != '') { ?>
    <?php the_category(); ?>
  <?php } ?>
  <div class="new-text">
